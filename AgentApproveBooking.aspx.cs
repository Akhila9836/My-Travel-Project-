...
public partial class AgentApproveBooking : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      // Session["auname"] = "ajotravel";
        ...
        if (!IsPostBack)
        {
            ...
            ob.dr = ob.ret_dr("select * from booking where auname='" + auname + "' and status=0 and Convert(date, tdate,104) >=getdate()");
           // ob.dr = ob.ret_dr("select * from booking where auname='" + auname + "' and status=0");
            while (ob.dr.Read())
            {
